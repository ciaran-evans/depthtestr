...
lcdVariantDepthObs <- function(sampB_obs, sampA, dists){
  N1 = nrow(sampA)

  new_dists <- sqrt(rowSums((sweep(sampA, 2, sampB_obs, "-"))^2))

  return(1/(N1 * (N1 - 1)) * lcdVariantDepthDists(new_dists, dists, N1))
}

depthFunLCDVariant <- function(sampB, sampA){
  dists <- as.matrix(stats::dist(sampA))
  apply(sampB, 1, function(x) lcdVariantDepthObs(x, sampA, dists))
}
...
#' @param method The depth method to use (one of "halfspace", "mahalanobis", "pvb", "lp", "pd", "lcd", "lcd-variant", or "custom")
...
depthTest <- function(sample1, sample2, method,
  ...
  depthFun <- switch(method,
                     ...
                     "pd" = depthFunPD,
                     "lcd" = depthFunLCD,
                     "lcd-variant" = depthFunLCDVariant,
                     "custom" = depthFun)
